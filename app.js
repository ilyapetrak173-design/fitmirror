// ================ FITMIRROR — ФИНАЛЬНАЯ ВЕРСИЯ ДЛЯ TIMEWEB CLOUD ================
...
// Используем PORT из переменной окружения (обязательно для Timeweb!)
...
// Загрузка товара (два фото + таблица)
...
app.post('/api/stylist-advice', (req, res) => {
  const { photo_analysis } = req.body;
  const colorType = photo_analysis?.color_type || 'неизвестный';
  const eyeColor = photo_analysis?.eye_color || 'неизвестный';
  res.setHeader('Content-Type', 'text/html; charset=utf-8');
  res.send(`
    <p><strong>Ваши данные:</strong> цветотип — ${colorType}, глаза — ${eyeColor}.</p>
    <p>Этот товар подчёркивает ваши сильные стороны. Советуем носить с...</p>
    <p style="font-size:12px; color:#777;"><em>(ИИ-рекомендации будут добавлены позже)</em></p>
  `);
});

// Запуск — ОБЯЗАТЕЛЬНО слушать 0.0.0.0
app.listen(PORT, '0.0.0.0', () => {
  console.log(`✅ FitMirror запущен на порту ${PORT}`);
});